...
int main(int argc, char *argv[])
{
  ...
  struct s_ncurses_dsp_monitor *p_alsa_read_monitor;
  ...
  p_alsa_read_monitor = ncurses_dsp_monitor_create(p_alsa_read_node, "ALSA INPUT");

  if(!p_alsa_read_monitor) goto cleanup_write;

  p_codec2_demod_monitor = ncurses_dsp_monitor_create(p_codec2_demod_node, "CODEC2 DATAC1 DEMODULATION");

  if(!p_codec2_demod_monitor) goto cleanup_alsa_mon;
  ...
  error = ncurses_dsp_monitor_throughput_start(p_alsa_read_monitor);
  ...
  error = ncurses_dsp_monitor_wait(p_alsa_read_monitor);
...
cleanup_alsa_mon:
  ncurses_dsp_monitor_cleanup(p_alsa_read_monitor);
