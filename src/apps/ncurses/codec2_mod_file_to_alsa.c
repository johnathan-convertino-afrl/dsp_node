...
int main(int argc, char *argv[])
{
  ...
  struct s_ncurses_dsp_monitor *p_alsa_write_monitor;
  ...
  p_alsa_write_monitor = ncurses_dsp_monitor_create(p_alsa_write_node, "ALSA WRITE");

  if(!p_alsa_write_monitor) goto cleanup_codec2;

  p_codec2_mod_monitor = ncurses_dsp_monitor_create(p_codec2_mod_node, "CODEC2 DATAC1 MODULATION");

  if(!p_codec2_mod_monitor) goto cleanup_alsa_mon;
  ...
  error = ncurses_dsp_monitor_throughput_start(p_alsa_write_monitor);
  ...
  error = ncurses_dsp_monitor_wait(p_alsa_write_monitor);
...
cleanup_alsa_mon:
  ncurses_dsp_monitor_cleanup(p_alsa_write_monitor);
