...
int main(int argc, char *argv[])
{
  ...
  // get args
  while((opt = getopt(argc, argv, "d:i:c:r:h")) != -1)
  {
    switch(opt)
    {
      case 'd':
  ...
  p_alsa_func_write_args = create_alsa_args(p_device_name, SND_PCM_FORMAT_U8, channels, rate);
  ...
  error = dsp_setInput(p_alsa_write_node, p_file_read_node);
